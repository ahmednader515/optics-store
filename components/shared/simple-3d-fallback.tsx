...
export default function Simple3DFallback({
  modelUrl,
  scale = 1,
  offsetX = 0,
  offsetY = 0,
  ...
  // Draw a simple 3D-like representation using 2D canvas
  useEffect(() => {
    ...
    // Add model URL indicator
    if (modelUrl) {
      ctx.fillStyle = '#10b981'
      ctx.font = '10px Arial'
      ctx.fillText('3D Model Loaded', 10, canvas.height - 10)
    }
    ...
    <Card className="w-full">
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle className="text-lg">معاينة النموذج ثلاثي الأبعاد (مبسطة)</CardTitle>
          <Button
            variant="outline"
            size="sm"
